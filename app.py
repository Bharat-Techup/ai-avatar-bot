...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:root123@localhost/ai_avatar'
...
def chat_with_pdf():
    ...
    if name and email:
        user = User(name=name, email=email, current_date=datetime.now(),ip_address=ip_address)
        db.session.add(user)
        db.session.commit()
